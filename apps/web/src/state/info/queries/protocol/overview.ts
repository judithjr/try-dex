...
const formatpattieswapFactoryResponse = (rawpattieswapFactory?: pattieswapFactory[]) => {
  if (rawpattieswapFactory) {
    return rawpattieswapFactory.reduce(
      (acc, cur) => {
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatpattieswapFactoryResponse(data?.pattieswapFactories)
      const overviewData24 = formatpattieswapFactoryResponse(data24?.pattieswapFactories)
      const overviewData48 = formatpattieswapFactoryResponse(data48?.pattieswapFactories)
